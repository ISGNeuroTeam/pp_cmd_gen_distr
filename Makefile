...
pack: make_build
	rm -f gen_distr-*.tar.gz
	echo Create archive \"gen_distr-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../gen_distr-$(VERSION)-$(BRANCH).tar.gz gen_distr

clean_pack:
	rm -f gen_distr-*.tar.gz


gen_distr.tar.gz: build
	cd make_build; tar czf ../gen_distr.tar.gz gen_distr && rm -rf ../make_build
...
dev: pp_cmd otl_v1_config.ini
	ln -s -r ./gen_distr pp_cmd/gen_distr
